...
- **Tier value, in order of preference:** (1) `pool_sample` — the mean insured value of the cards the platform lists in that tier's public prize pool at snapshot time, with the sample size shown (Collector Crypt exposes this; its pool endpoint pages at ~40 cards per tier, so it is a sample, not the whole pool); (2) `band_midpoint` — the midpoint of the published value band, an open-ended band ("$200+") valued at its floor. The method is named on every row; mixing methods within one pack raises a warning.
- **The platform's own EV**, where it publishes one, is shown beside ours as `stated_ev` with its implied `stated_edge = 1 − buyback_pct × stated_ev / price`. It is the platform's number under the platform's undisclosed method; we do not use it in our house edge. On 2026-09-26 the $3,000 Collector Crypt machine read: stated EV 3,032; our pool-sample EV 3,860; band-midpoint 4,598 — the spread between those three is itself information.
