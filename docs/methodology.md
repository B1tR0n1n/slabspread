...
- **Tier value, in order of preference:** (1) `pool_full` — the mean insured value over *every* card the platform lists in that tier's public prize pool at snapshot time (Collector Crypt exposes the pool, paged, value-descending; we walk all pages and record the count against the platform's stock figure); (2) `pool_sample` — the same mean when the walk was incomplete, with the sample size shown — treat as an upper bound because the pool is served highest-value-first; (3) `band_midpoint` — the midpoint of the published value band, an open-ended band ("$200+") valued at its floor. The method is named on every row; mixing methods within one pack raises a warning.
- **Within-tier draw is assumed uniform.** Validation: on 2026-09-26 the full-pool uniform mean for Collector Crypt's $3,000 machine was 3,031.19 against the platform's stated EV of 3,031.82 (ratio 0.9998; one common card was missing from our walk). That agreement is the evidence for the assumption, and it will be re-checked on every snapshot by comparing `ev_platform` to `stated_ev`.
- **The platform's own EV**, where it publishes one, is shown beside ours as `stated_ev` with its implied `stated_edge = 1 − buyback_pct × stated_ev / price`. It is the platform's number under the platform's undisclosed method; we do not use it in our house edge. On 2026-09-26 the $3,000 Collector Crypt machine read: stated EV 3,032; our full-pool EV 3,031; a 40-card top-of-pool sample 3,860; band-midpoint 4,598 — the last two show why a partial sample or a band is not a value.
